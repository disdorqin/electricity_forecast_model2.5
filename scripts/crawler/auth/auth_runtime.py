...
def _validate_cookie(cookie: str, cfg: dict[str, Any]) -> bool:
    if not cookie or len(cookie) < 20:
        return False
    try:
        from scripts.crawler.auth.browser_session import validate_cookie
...
def ensure_authenticated_config(
    ...
    *,
    ...
    if auth_mode in {"auto", "account"} and _has_credentials(cfg):
        ...
        try:
            from scripts.crawler.auth.auth import PmosAuth
    ...
    if auth_mode in {"auto", "browser"}:
        ...
        try:
            from scripts.crawler.auth.browser_session import ensure_browser_cookie
