...
from integrate_paper_modules_p6_strict import build_strict_quantiles  # noqa: E402
...
def main() -> None:
    ...
    q = build_strict_quantiles(slot, p6, all_days, days, args.training_days, args.seed)
    atomic_parquet(root / args.output, q)
    print(f"strict cache days={q.target_day.nunique()} rows={len(q)} range={q.target_day.min()}..{q.target_day.max()}")
